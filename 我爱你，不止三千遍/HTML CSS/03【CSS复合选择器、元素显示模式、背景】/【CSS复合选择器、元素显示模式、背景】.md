...
## 1.5 交集选择器
`交集选择器` 选中页面中**同时满足**多个选择器的标签。
`交集选择器` 是各选择器通过**英文句号** `.` 连接而成，任何形式的选择器都可以作为并集选择器的一部分。
...
上述语法表示找到页面中**既**能被选择器1选中，**又**能被选择器2选中的标签。
注意1：交集选择器中的选择器之间是紧挨着的，没有东西分隔。
注意2：交集选择器中如果有标签选择器，标签选择器必须写在最前面。

**例如：**

```css
p.box { 样式声明 }		 /* 找到p标签，而且p标签添加了box类 */
```
...
| 交集选择器      | 选择同时符合样式的元素 | 可以用于集体声明 | 较多     | 紧挨着写             |
